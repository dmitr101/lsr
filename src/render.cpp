...
void triangle_line_sweeping(image_utils::pixel const &color, image::pos const &p1, image::pos const &p2,
                            image::pos const &p3, image &target)
{
...
void triangle_entire_bbox(image_utils::pixel const &color, image::pos const &p1, image::pos const &p2,
                          image::pos const &p3, image &target)
{
}
...
model::face apply_transform(model::face const &triangle, glm::mat4x4 const &transform)
{
    ...
    std::transform(begin(triangle), end(triangle), begin(result),
                   [&transform](auto const &v) { return apply_transform(v, transform); });
    return result;
}
...
array<image::pos, 3> to_screen_coords(model::face const &triangle, size_t const width, size_t const height)
{
    ...
    std::transform(begin(triangle), end(triangle), begin(result),
                   [width, height](auto const &v) { return to_screen_coords(v, width, height); });
    return result;
}
} // namespace
