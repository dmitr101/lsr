...
bool save_to_png(image const &img, std::string_view file_name) noexcept
{
    ...
    return stbi_write_png(file_name.data(), static_cast<int>(img.width()), static_cast<int>(img.height()),
                          static_cast<int>(std::tuple_size_v<image::pixel>), img.data_raw(), 0) != 0;
}
} // namespace image_utils
...
image::image(size_t const width, size_t height) noexcept : data_(width * height), width_(width), height_(height)
{
}
...
image::data_container &image::data() noexcept
{
    return data_;
}
...
{
    return data_.empty() ? nullptr : reinterpret_cast<pixel_component const *>(data_.data());
}
